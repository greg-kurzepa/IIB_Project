...
res = _model_springs.solve_springs4(pile, soil, P, 3, N, tol=1e-10)
P_ult = res.P_cap

#%%

forces = np.linspace(0, P_ult, 50)[1:-1]

head_settlements = []
base_settlements = []
for force in tqdm(forces):
    res = _model_springs.solve_springs4(pile, soil, force, 3, N)
    head_settlements.append(res.d[0])
    base_settlements.append(res.d[-1])

#%% plot head settlements v force

plt.figure(figsize=(10, 6))
plt.plot(np.array(head_settlements)*1000, forces/1e6)
plt.xlabel("Head Settlement (mm)")
plt.ylabel("Force (MN)")
plt.grid()
plt.show()

#%% plot base settlements v force

plt.figure(figsize=(10, 6))
plt.plot(np.array(base_settlements)*1000, forces/1e6)
plt.xlabel("Base Settlement (mm)")
plt.ylabel("Force (MN)")
plt.grid()
plt.show()
