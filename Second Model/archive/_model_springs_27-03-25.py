...
def solve_springs4(pile, soil, P, z_w, N=100, t_res_clay=0.9,
    ...
    res, infodict, ier, mesg = scipy.optimize.fsolve(f_simultaneous, initial, xtol=tol, full_output=True,
                                                     args=(dz, N, pile, A, P, Q_ult, tau_ult, Q_over_Q_ult_func, tau_over_tau_ult_func, tau_limits, Q_limit))
...
def solve_elastic(pile, E_soil, P, N=100, tol=1e-8, outtol=1e-10):
    ...
    if any(abs(f_simultaneous_elastic(res, dz, N, pile, E_soil, P)) > outtol):
        print(f"WARNING: Solver did not converge, fsolve error was >1e-10. ier: {ier}, mesg: {mesg}")
